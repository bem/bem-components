...
* `touch.blocks` — реализация особенностей для touch-платформ.
* `touch-phone.blocks` — реализация особенностей для смартфонов.
* `touch-pad.blocks` — реализация особенностей для планшетов.
* `design/<common|desktop|touch|touch-phone|touch-pad>.blocks` — реализация различных дизайнов (тем оформления).

> Подробнее про [уровни переопределения](https://github.com/bem-site/bem-method/blob/bem-info-data/method/redefinition-levels/redefinition-levels.ru.md).
...
Библиотеку можно использовать тремя способами:

| <h3>Dist</h3> | <h3>Source</h3> | <h3>Compiled</h3> |
|------|--------|----------|
...
| [Подключение](#Подключение-предсобранных-файлов-библиотеки-dist) | [Подключение](#Подключение-исходного-кода-библиотеки-source-и-compiled) | [Подключение](#Подключение-исходного-кода-библиотеки-source-и-compiled) |

### Подключение исходного кода библиотеки (Source и Compiled)

Рекомендуемый способ подключения — использование инструментов [ENB](https://github.com/enb/enb/blob/master/README.ru.md) или [bem-tools](https://ru.bem.info/tools/bem/bem-tools/).
...
Самый простой способ подключить библиотеку в проект — скачать предварительно собранные файлы библиотеки и добавить их в HTML страницы с помощью элементов `<link>` и `<script>`.
...
Бандлы без автоматической инициализации. Следует использовать, чтобы добавлять собственный код на `i-bem.js`:
* bem-components.no-autoinit.js - JavaScript
...
* [Собрать файлы из исходного кода](#Сбор-файлов-из-исходного-кода) — возможность собрать еще не выпущенную версию библиотеки.
...
Схема подключения файла с CDN:

`//yastatic.net/название-библиотеки/версия/платформа/имя-файла`.

Пример:

`//yastatic.net/bem-components/latest/desktop/bem-components.dev.js+bemhtml.js`.
...
<script>
modules.require(['i-bem-dom', 'BEMHTML', 'jquery'], function(BEMDOM, BEMHTML, $, init) {
