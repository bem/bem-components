...
Модификатор `type` в значении `link` используется для изменения поведения кнопки: при нажатии кнопка ведет себя как ссылка. Переход осуществляется по адресу, указанному в поле <a href="#buttonurl">url</a>.
...
Модификатор `type` в значении `submit` используется для создания кнопки отправки данных на сервер. Кнопка такого типа всегда должна располагаться в форме.
