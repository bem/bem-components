...
**Важно!** Несмотря на то, что поле `content` также позволяет определить текст кнопки или любой другой произвольный BEMJSON вместо элемента `button__text`, так делать не рекомендуется и лучше использовать поле `text`.
