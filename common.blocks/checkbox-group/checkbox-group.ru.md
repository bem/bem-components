...
Помимо обязательных параметров в массиве `option` могут быть переданы любые параметры, допустимые для блока `checkbox`, например, модификаторы `checked` или `disabled`.
...
### Размер переключателей `size`

Обязательный модификатор. Модификатор `size` устанавливает размер переключателей для всех типов групп чекбоксов.
...
<table>
    <tr>
        <th>Размер</th>
        <th>Обычная группа чекбоксов и <code>checkbox-group_type_line</code></th>
        <th>Кнопочная группа чекбоксов (<code>checkbox-group_type_button</code>)</th>
    </tr>
    <tr>
        <th>s</th>
        <td>–</td>
...
Размеры на примере темы *normal* с модификатором `checkbox-group_type_button`:
...
### Тип группы чекбоксов `type`

Доступны следующие значения модификатор `type`:

* Модификатор `type` со значением `button`. Используется для создания кнопочной группы чекбоксов – блока из кнопок-переключателей. 
* Модификатор `type` со значением `line` используется для создания группы чекбоксов с выравниванием в строку. После каждого переключателя группы, кроме последнего, автоматически добавляется отступ справа. Размер отступа зависит от значения модификатора `size`. Реализован только в теме *normal*.
...
Такая группа чекбоксов не будет получать фокус (модификатор `focused`).

При установке модификатора `disabled` для группы всем переключателям группы также устанавливается модификатор `disabled`. Как следствие, для них не будут:
