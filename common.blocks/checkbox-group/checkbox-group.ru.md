...
* `val` (`String`|`Number`) – значение, возвращаемое контролом переключателя группы чекбоксов (элемент `checkbox__control`), если переключатель выбран;
...
* Модификатор `_type` со значением `line` используется для создания группы чекбоксов с выравниванием в строку. После каждого переключателя группы, кроме последнего, автоматически добавляется отступ справа. Размер отступа зависит от значения модификатора `_size`.

Модификаторы реализован во всех темах блока.
