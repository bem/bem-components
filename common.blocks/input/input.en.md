...
The **input** block is intended to create the different types of text fields:

* textarea;
* password field;
* search form.

In MSIE8 block **input** degrades as native control `<input attribute>`; a text field is renedered as native text area `<textarea attribute>`.
...
<table>
  ...
  <tr>
    <td>placeholder</td>
    <td>String</td>
    <td>Text to be replaced. Has an empty value.</td>
  </tr>
  <tr>
    <td>id</td>
    <td>String</td>
    <td>Unique block modifier. The value is  По умолчанию генерируется случайным образом, если явно не задан в BEMJSON.
  </tr>
  <tr>
    <td>label</td>
    <td>`string`</td>
...
It disabled modifier is not specified the **input** block is active by default.
