...
| ---- | --- | -------- |
| <a href=#name>name</a> | <code>String</code> | `radio-group` name that is used for its identification. Specifies `name` HTML attribute to a nested `input` block. |
| <a href=#options>options</a> | <code>Array</code> | An array of pairs that maps `text` to `val` for each switch in radio-group. Each pair `name=val` within the array corresponds to one radio switch. Pair `name=val` is sent to a server, where name of a radio group is set by `name` field and value of each radio switch – by `val` field. 
