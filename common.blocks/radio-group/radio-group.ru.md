...
Блок `radio-group` в результате применения шаблонов становится на странице HTML-элементом с тегом `<span>` – контейнером, содержащим группу переключателей (радиокнопок).
...
<table>
    <tr>
        ...
        <th>Кнопочная радиогруппа (<code>radio-group_type_button</code>)</th>
    </tr>
    <tr>
        <th>s</th>
        <td>–</td>
            ...
            <pre><code>
{
    ...
    <tr>
        <td>Строчная радиогруппа (<code>radio-group_type_line</code>)</td>
        <td>
            <pre><code>
{
    block : 'radio-group',
...
### Режим выбора элементов группы `mode`

Модификатор `mode` управляет реакцией элементов радиогруппы на щелчек мыши, например, возможностью деактивации выбранного элемента. Модификатор доступен только при установленном модификаторе `radio-group_type_button`.

Доступны следующие значения модификатора:

* `radio-check` – режим радио чекбокса. При нажатии на радиопереключатель он активируется, при повторном нажатии деактивируется. Например:
